    ...
    const shareWebsite = async () => {
        ...
        try {
            if (navigator.share) {
                ...
                copyWebsiteUrl();
                alert(
                    "이 브라우저는 공유 기능을 지원하지 않습니다. URL이 복사되었습니다."
                );
            }
        } catch (error) {
            ...
            <AccountCards>
                ...
                <AccountCard onClick={() => toggleSide("groom")}>
                    ...
                    {expandedSide === "groom" && (
                        <AccountRowsContainer>
                            {renderAccountRow(
                                weddingConfig.account.groom,
                                "groom",
                                "신랑"
                            )}
                            {renderAccountRow(
                                weddingConfig.account.groomFather,
                                "groomFather",
                                "아버지"
                            )}
                            {renderAccountRow(
                                weddingConfig.account.groomMother,
                                "groomMother",
                                "어머니"
                            )}
                        </AccountRowsContainer>
                ...
                <AccountCard onClick={() => toggleSide("bride")}>
                    ...
                    {expandedSide === "bride" && (
                        <AccountRowsContainer>
                            {renderAccountRow(
                                weddingConfig.account.bride,
                                "bride",
                                "신부"
                            )}
                            {renderAccountRow(
                                weddingConfig.account.brideFather,
                                "brideFather",
                                "아버지"
                            )}
                            {renderAccountRow(
                                weddingConfig.account.brideMother,
                                "brideMother",
                                "어머니"
                            )}
                        </AccountRowsContainer>
...
const AccountCardHeader = styled.div<{ $isExpanded: boolean }>`
    ...
    padding: 1.25rem;
    border-bottom: ${(props) =>
        props.$isExpanded ? "1px solid #eee" : "none"};
`;
...
const ExpandIcon = styled.span<{ $isExpanded: boolean }>`
    ...
    transform: ${(props) =>
        props.$isExpanded ? "rotate(0deg)" : "rotate(0deg)"};
`;
