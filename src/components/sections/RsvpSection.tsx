    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setSubmitStatus({
                success: false,
                message:
                    "참석 여부 전송 중 오류가 발생했습니다. 다시 시도해 주세요.",
            });
        } finally {
...
const StatusMessage = styled.div<{ $success: string }>`
    ...
    font-size: 0.9rem;
    background-color: ${(props) =>
        props.$success === "true" ? "#e7f3eb" : "#fbedec"};
...
const AttendanceButton = styled.button<{ $selected?: boolean }>`
    ...
    padding: 0.75rem;
    border: 1px solid
        ${(props) => (props.$selected ? "var(--secondary-color)" : "#ccc")};
    border-radius: 4px;
    background-color: ${(props) =>
